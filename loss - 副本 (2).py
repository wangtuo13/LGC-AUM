...
class lgc_loss(nn.Module):
    ...
    def loss(self, q1, q2, D1, D2, one=True, pur=False):
        if one:
            if pur:
                ...
                return torch.mean(
                    torch.sum(D1 * torch.log(1 / q1), dim=1) +
                    torch.sum((1 - D2 + self.eps) * torch.log(1 / (1 - q2 + self.eps)), dim=1)
                )
        else:
            if pur:
    ...
    def load_model(self, path):
        ...
        #pretrained_dict = {k: v for k, v in pretrained_dict['lgc_loss'].items() if k in model_dict}
    ...
    def maintain_similarity(self, z1, z0, zbatch, zbatch1, zbatch0, epoch, batch_idx, batch_size):
        wide = z1.size(0)
