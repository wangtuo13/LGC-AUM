...
class lgc_loss(nn.Module):
    ...
    def Similarity_Mantain_Norm(self, zbatch, zbatch1, z, batch_idx, batch_size, num, epoch, m, divide):
        d_z = torch.sum((z.unsqueeze(1) - z + self.eps) ** 2, dim=2).sqrt()
        d_x = torch.sum((zbatch.unsqueeze(1) - zbatch + self.eps) ** 2, dim=2).sqrt()
        d_z_0 = torch.sum((zbatch1.unsqueeze(1) - zbatch1 + self.eps) ** 2, dim=2).sqrt()

        d_z_u = torch.sum((zbatch1.unsqueeze(1) - self.mu.detach() + self.eps) ** 2, dim=2).sqrt()
        ...
        Relation_z = d_z_0.topk(k=2, dim=1, largest=False)
        Relation_x_u = d_.topk(k=self.mu.size(0), dim=1, largest=False)
        Relation_z_u = d_z_u.topk(k=self.mu.size(0), dim=1, largest=False)
        ...
        if epoch >= 0:
            ...
            for i in range(self.mu.size(0) * 30):
                ...
                current_z = torch.sum((-(self.relu(Relation_z_u[0] - rho_z) / sigma_z)).exp(), dim=1)
                mid_z[range(batch_size), (current_z > target) * 1] = sigma_z[:, 0]
                sigma_z = torch.mean(mid_z, dim=1).unsqueeze(1)
