...
def train():
    ...
    for epoch in range(Data.pretrain_epoch):
        ...
        small_z = []
        Y = []
        for batch_idx, ((x, x1, _, _), y) in enumerate(train_loader):
            if Data.net == 'CNN_VAE3':
                #x = x.to(device).view(-1, Data.channel, Data.wide, Data.wide)
                #x_noise = add_noise(x, Data.noise)
                #x_rec = autoencoder(x_noise)

                x = x.to(device).view(-1, Data.channel, Data.wide, Data.wide)
                x_noise1 = add_noise(x, Data.noise)
                x_rec1 = autoencoder(x_noise1)

                if epoch >= 500 and batch_idx<2:
                    to_pil_image = transforms.ToPILImage()
                    print('x')
                    plt.imshow(to_pil_image(x[0] * 200))
                    plt.show()

                    print('x_noise')
                    plt.imshow(to_pil_image(x_noise[0] * 200))
                    plt.show()

                    print('x1')
                    plt.imshow(to_pil_image(x1[0] * 200))
                    plt.show()

                    print('x_noise1')
                    plt.imshow(to_pil_image(x_noise1[0] * 200))
                    plt.show()

                    print('x_rec1')
                    plt.imshow(to_pil_image(x_rec1[0] * 200))
                    plt.show()
            ...
            loss = F.mse_loss(x_rec1, x)  #+ #F.mse_loss(x_rec1, x) #+ F.mse_loss(x_rec1, x)
        ...
        if epoch % 1 == 0:
            for _, ((x, x1, _, _), y) in enumerate(val_loader):
                if Data.net == 'CNN_VAE3':
                    x = x.to(device).view(-1, Data.channel, Data.wide, Data.wide)
                    small_z.append(autoencoder.encode(x)[0].data)
                    Y.append(y.data)
                elif Data.net == 'VAE':
                    x = x.to(device).view(-1, Data.input_size)
        ...
        if epoch + 1 == 50 or epoch + 1 == 100 or epoch + 1 == 150 or epoch + 1 == 200 or epoch + 1 == 250 or epoch + 1 == 300 or epoch + 1 == 350 or epoch + 1 == 400 or epoch + 1 == 450 or epoch + 1 == 500:
            if Data.net == 'CNN_VAE3':
                pretrain_dir = '../pretrain_cnn3_model/pre_cnn_'
            else:
                pretrain_dir = '../pretrain_model/pre_vae_'
            pretrain_model = Data.name + '_Acc_' + str(round(Acc * 100, 2)) + '_hidim_' + str(
                Data.hidden_dim) + '_' + str(Data.pretrain_optim) \
                             + '_lr_' + str(Data.pretrain_lr) + '_n_work_' + str(Data.num_workers) \
                             + '_noise_' + str(Data.noise) + '_btsize_' + str(Data.pretrain_batchsize) + '_epoh_' + str(
                epoch + 1) + '.pt'
...
if __name__ == "__main__":
    ...
    Data = Data_style(name=args.name, input_size=args.input_size, n_cluster=args.n_cluster, hidden_dim=args.hidden_dim,
                      shuffle=args.shuffle,
                      pretrain=args.pretrain, pretrain_batchsize=args.pretrain_batchsize,
                      pretrain_epoch=args.pretrain_epoch,
                      pretrain_lr=args.pretrain_lr, pretrain_optim=args.pretrain_optim, noise=args.noise,
                      trans=args.trans,
    ...
    elif Data.net == 'CNN_VAE3':
        autoencoder = CNN_VAE3(net="resnet18", hidden=Data.hidden_dim, cluster_num=Data.n_cluster).to(device)
        print('\nencoder: ', autoencoder.encoder, '\ndecoder: ', autoencoder.decoder, '\n')
    elif Data.net == 'VAE':
    ...
    if Data.net == 'CNN_VAE3':
        ...
        pretrain_dir = '../pretrain_model/pre_vae_'
    pretrain_model = Data.name + '_Acc_' + str(round(Acc * 100, 2)) + '_hidim_' + str(Data.hidden_dim) + '_' + str(
        Data.pretrain_optim) \
                     + '_lr_' + str(Data.pretrain_lr) + '_n_work_' + str(Data.num_workers) \
                     + '_noise_' + str(Data.noise) + '_btsize_' + str(Data.pretrain_batchsize) + '_epoh_' + str(
        Data.pretrain_epoch) + '.pt'
    path = pretrain_dir + pretrain_model
    torch.save(autoencoder.state_dict(), path)
    print('\nsaving pretrain weight to... ', path)
    record.plot(path, args.plot)
