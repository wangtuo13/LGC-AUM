...
kmeans0 = KMeans(Data.n_cluster, n_init=3)
...
count = 100*np.ones(Data.n_cluster, dtype=np.int)
for epoch in range(Data.cluster_epoch):
    ...
    if (epoch + 1) % 1 == 0:
        y_pred = kmeans1.fit_predict(z.data.cpu().numpy())
        manifold.mu.data.copy_(torch.Tensor(kmeans1.cluster_centers_))
    if epoch==0:
    ...
    if Y is not None:
        ...
        y_pred1 = y_pred  #torch.argmax(D, dim=1).data.cpu().numpy()
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        labels = y_pred[batch_idx * Data.cluster_batchsize: min((batch_idx + 1) * Data.cluster_batchsize, Data.total)]
        centers_batch_tensor = manifold.mu[labels]
        ...
        if stage == 'local':
            ...
            ''' New Method '''
            if Data.net == 'VAE':
                optimizer = optim.Adam([{'params': manifold.mu}, {'params': lgc.encoder2.parameters()}], lr=Data.cluster_lr)
            elif Data.net == 'CNN_VAE2':
                optimizer = optim.Adam([{'params': manifold.mu}, {'params': lgc.encoder.parameters()}],  lr=0.0001)
            z = lgc(x, local=True)
            x_rec = lgc.decoder(z)
            recon_loss = F.mse_loss(x_rec, x)
            cluster_loss = torch.mean(torch.sum((centers_batch_tensor - z) ** 2, 1))
        ...
        #temp_idx= kmeans1.fit_predict(z.data.cpu().numpy()) #batch_km(z.data.cpu().numpy(), manifold.mu, count)
        temp_idx, temp_mu, count = batch_km(z.data.cpu().numpy(), manifold.mu.detach().cpu().numpy(), count)
        manifold.mu.data.copy_(torch.Tensor(temp_mu))
        y_pred[batch_idx * Data.cluster_batchsize: min((batch_idx + 1) * Data.cluster_batchsize, Data.total)] = temp_idx
...
if Data.net == 'VAE':
    final_dir = '../final_model/F_DCN_'
elif Data.net == 'CNN_VAE2':
    final_dir = '../final_CNN_model/FC_DCN_'
