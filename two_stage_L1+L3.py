...
def train():
    ...
    for global_epoch in range(args.global_epoch, Global_Max):
            ...
            AD = Evaluation_single(p, y_ground)
            '''
            ...
            Nmi_AD, Ari_AD = Evaluation2(y_ground, D)

            print("Global_stage: %d, L_Global: %.4f, AK: %.4f, AC: %.4f, Nmi: %.4f, Ari: %.4f, Learned AD: %.4f, NMI by AD: %.4f, ARI by AD: %.4f" % (
                global_epoch, total_loss / (batch_idx + 1), AK, AC, Nmi, Ari, AD, Nmi_AD, Ari_AD))
